      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
          {(Array.isArray(experienceStyles) ? experienceStyles : []).map((style, index) => {
            const translatedLabel = t(`experienceStyles.${style.id}.label`, { defaultValue: style.label });
            const translatedDesc = t(`experienceStyles.${style.id}.description`, { defaultValue: style.description });
            ...
            return (
              <motion.button
                ...
                aria-label={`${translatedLabel} - ${translatedDesc}`}
              >
                {/* Background image */}
                <img
                  src={style.image}
        ...
        >
          {t('experienceSelector.clickInstruction')}
        </motion.p>
      </div>
    </section>
  );
