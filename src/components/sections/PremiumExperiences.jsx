...
const PremiumExperiences = () => {
  ...
  const premiumTours = toursData.filter((tItem) => tItem.requireWhatsApp);
  ...
  const premiumMeta = {
    'tour-helicoptero': {
      icon: '🚁',
      highlight: t('premium.helicopter.highlight', { defaultValue: 'Vista 360° de Jericoacoara' }),
      detail: t('premium.helicopter.detail', { defaultValue: 'Sobrevoe as dunas, lagoas e o litoral paradisíaco do alto, numa experiência única e inesquecível.' }),
      bgGradient: 'from-slate-900 via-slate-800 to-gray-900',
      accentColor: '#D4AF37',
    },
    'tour-utv': {
      icon: '🏎️',
      highlight: t('premium.utv.highlight', { defaultValue: 'Off-Road nas Dunas' }),
      detail: t('premium.utv.detail', { defaultValue: 'Adrenalina e segurança em um passeio exclusivo de UTV pelas trilhas e dunas de Jericoacoara.' }),
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {(Array.isArray(premiumTours) ? premiumTours : []).map((tour, index) => {
            const meta = premiumMeta[tour.id] || {
              icon: '✨',
              highlight: t('drawer.premiumCategory'),
              ...
              >
                ...
                <div className="relative z-10 flex flex-col flex-1 p-8 md:p-10 justify-end">
                  ...
                  <a
                    ...
                    aria-label={`${t('premium.consultBtn')} - ${displayTitle}`}
                  >
