        ...
        {viewMode === 'lista' && (
          <div className="space-y-4">
            {reservations.length === 0 ? (
              ...
              reservations.map((r) => {
                const serviceTitle = getReservationServiceName(r);
                const pickupData = parsePickupLocation(r.pickup_location || r.notes || '', r.remaining_balance, r.origin);
                ...
                const isSinalPago = r.payment_status === 'sinal_pago' || (r.remaining_balance > 0 && r.amount_paid > 0);
                ...
                return (
                  ...
                  >
                    ...
                    <div className="space-y-2 min-w-[240px]">
                      ...
                      <div className="pt-1">
                        <span className="inline-flex items-center gap-1 text-[10px] font-medium text-slate-600 bg-slate-100 px-2 py-0.5 rounded-full border border-slate-200">
                          🌐 {pickupData.channel || r.origin || 'Site Institucional'}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 flex-1 border-t lg:border-t-0 lg:border-l border-slate-100 pt-3 lg:pt-0 lg:pl-5">
                      {/* Título do Serviço / Pacote Resumido */}
                      <div className="flex items-center gap-2 text-sm font-bold text-slate-900 border-b border-slate-100 pb-1.5">
                        <Compass className="w-4.5 h-4.5 text-emerald-600 shrink-0" />
                        <span>{serviceTitle}</span>
                      </div>

                      {r.items.map((it, idx) => (
                        <div key={it.id || idx} className="space-y-1 pt-1">
                      ...
                      {/* Logística / Linha de Embarque Sanitizada */}
                      <div className="pt-2 space-y-1.5 border-t border-slate-100 mt-2">
                        <p className="text-xs text-slate-800 font-semibold flex items-center gap-1.5">
                          ...
                          <span>Embarque: <strong>{pickupData.physicalLocation}</strong></span>
                        </p>
                        <div className="flex flex-wrap items-center gap-1.5 text-[10px]">
                          {pickupData.route && (
                            <span className="px-2 py-0.5 rounded bg-indigo-50 text-indigo-700 border border-indigo-200 font-bold">
                              Trajeto: [{pickupData.route}]
                            </span>
                          )}
                          {pickupData.channel && (
                            <span className="px-2 py-0.5 rounded bg-slate-100 text-slate-700 border border-slate-200 font-semibold">
                              Canal: [{pickupData.channel}]
                            </span>
                          )}
                          {r.remaining_balance > 0 && (
                            <span className="px-2 py-0.5 rounded bg-amber-100 text-amber-900 border border-amber-300 font-extrabold">
                              Saldo: [Cobrar {formatMoney(r.remaining_balance)} no embarque]
                            </span>
                          )}
                        </div>
                      </div>
                    ...
                    {/* Coluna 3: Cabeçalho Financeiro Limpo */}
                    <div className="space-y-1.5 min-w-[220px] border-t lg:border-t-0 lg:border-l border-slate-100 pt-3 lg:pt-0 lg:pl-5">
                      <div className="text-xs text-slate-600 font-medium">
                        Total: <strong className="text-slate-900 text-sm">{formatMoney(r.amount_total)}</strong>
                      </div>
                      <div className="text-xs text-emerald-700 font-semibold">
                        Pago: <strong className="text-emerald-800">{formatMoney(r.amount_paid)}</strong>
                      </div>
                      <div className="text-xs text-blue-800 font-bold">
                        A Receber no Embarque: {formatMoney(r.remaining_balance)}
                      </div>

                      {/* Destaque do Badge de Status Financeiro */}
                      <div className="pt-1.5">
                        {isQuitado ? (
                          <span className="inline-flex items-center gap-1 text-xs font-bold text-emerald-800 bg-emerald-100 px-2.5 py-1 rounded-md border border-emerald-300">
                            <Check className="w-3.5 h-3.5 text-emerald-700" /> Quitado (100%)
                          </span>
                        ) : isSinalPago ? (
                          <span className="inline-flex items-center gap-1 text-xs font-bold text-amber-900 bg-amber-100 px-2.5 py-1 rounded-md border border-amber-300">
                            🏷️ [Sinal 50% Pago ({r.payment_method === 'cartao' ? 'Cartão/MP' : 'Pix'})]
                          </span>
                        ) : (
